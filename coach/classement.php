...
<div class="card table-card">
    <div class="card-body p-0 table-responsive">
        ...
        <table class="table mb-0">
            ...
            <tr class="<?= $isOurs?'table-success':'' ?>">
                ...
                <td class="fw-semibold<?= $isOurs?' text-success':'' ?>">
                    <?= e($row['equipe']) ?><?= $isOurs?' <span class="badge bg-success ms-1">Nous</span>':'' ?>
                </td>
                ...
                <td class="text-center <?= ($row['bp']-$row['bc'])>0?'text-success':(($row['bp']-$row['bc'])<0?'text-danger':'') ?>">
        ...
        <?php else: ?>
            <div class="text-center py-5 text-muted"><i class="bi bi-award fs-1 d-block mb-3"></i>Aucun résultat pour cette compétition.</div>
        <?php endif; ?>
    </div>
</div>
