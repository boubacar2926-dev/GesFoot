...
$stmt->execute([$id]);
$conv = $stmt->fetch();
if (!$conv) { setFlash('error','Convocation introuvable.'); redirect('/admin/convocations/index.php'); }

$joueurs = $pdo->prepare("
    SELECT j.*, cj.statut AS conv_statut
    FROM convocation_joueur cj
    JOIN joueurs j ON j.id = cj.joueur_id
    WHERE cj.convocation_id = ?
...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div class="ms-auto d-flex gap-2">
        <a href="<?= BASE_URL ?>/admin/convocations/add.php?id=<?= $conv['id'] ?>" class="btn btn-sm btn-outline-primary">
...
<div class="row g-4">
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-header"><i class="bi bi-calendar-event me-2"></i>Détails du match</div>
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <div class="fs-4 fw-bold">vs <?= e($conv['adversaire']) ?></div>
                    <?= statusBadge($conv['statut']) ?>
                </div>
                ...
                <h6 class="fw-bold">Informations RDV</h6>
                <ul class="list-unstyled small">
                    <?php if ($conv['date_convocation']): ?><li class="mb-1"><i class="bi bi-calendar-check text-muted me-2"></i><?= formatDate($conv['date_convocation']) ?></li><?php endif; ?>
                ...
                <div class="mt-3 pt-2 border-top">
                    <span class="badge bg-primary fs-6"><?= count($joueurs) ?> joueur(s) convoqué(s)</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header"><i class="bi bi-people me-2"></i>Liste des convoqués</div>
            <div class="card-body p-0 table-responsive">
                <table class="table mb-0">
                    <thead><tr><th>#</th><th>Joueur</th><th>Poste</th><th>N°</th><th>Statut</th></tr></thead>
                    <tbody>
                    <?php if ($joueurs): ?>
                        ...
                        <tr>
                            ...
                            <td><?= statusBadge($j['conv_statut']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="5" class="text-center py-4 text-muted">Aucun joueur convoqué</td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
